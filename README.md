...
1.  **リアルタイム高精度メーター**:
    *   `FusedLocationProviderClient` を利用した高精度 100ms（10Hz）周期の位置・速度情報取得。
    *   `location.speed` を優先使用し、5サンプルの移動平均フィルタでノイズを低減させた安定した速度表示。
...
1. **Real-time High-Precision Speedometer**:
    * Utilizes `FusedLocationProviderClient` for high-accuracy 100ms (10Hz) location and speed acquisition.
    * Prioritizes `location.speed` and applies a 5-sample moving average filter to reduce noise for stable speed display.
