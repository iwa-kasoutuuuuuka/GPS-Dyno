...
GPS Dyno は、位置情報や走行データという非常に機密性の高いプライベートデータを扱うため、アーキテクチャレベルで堅牢なセキュリティ設計を施しています。

*   **インターネット接続の完全不使用 (外部漏洩リスクゼロ)**:
    *   本アプリは `android.permission.INTERNET`（インターネットアクセス権限）を要求していません。データが外部のサーバーに送信されるリスクは **物理的に 100% 存在しません。**
*   **強固なローカルサンドボックス保護 (SQLインジェクション対策)**:
    *   走行データは Android OS のセキュリティ機構に基づき、アプリ専用の隔離されたデータ領域（SQLite/Room）に安全に保存されます。他アプリからアクセスすることは不可能です。
    *   Roomの採用により、クエリのパラメータバインド（プリペアドステートメント）が徹底されており、**SQLインジェクション脆弱性は根本的に排除** されています。
*   **FileProviderによる安全なエクスポート (ディレクトリ・トラバーサル防止)**:
    *   CSVの共有には `FileProvider` を使用し、アクセス範囲を専用キャッシュ（`exports/`）のみに厳密に制限。他アプリに内部データベースや機密ファイルを覗き見される「ディレクトリ・トラバーサル脆弱性」を完全に防ぐ最新のベストプラクティスに準拠しています。
*   **コンポーネントの非公開化 (サービス乗っ取りの防止)**:
    *   バックグラウンドロギングを担当する `GPSLoggingService` は `exported="false"` に設定されており、外部の悪意あるアプリから不正にコントロールされるリスクを排除しています。
...
Since GPS Dyno handles highly sensitive location and driving metrics, it features a robust, secure-by-default architecture:

*   **Zero Internet Permission (Leak-Proof)**:
    *   The app does not request or possess `android.permission.INTERNET`. Driving and location data can **never be sent to external servers (100% leak-proof by design).**
*   **Local Sandbox & Prepared Statements (SQL Injection Immunity)**:
    *   All telemetry logs are stored in an app-private storage (SQLite/Room) backed by the Android OS user-separation security model.
    *   By using Room, all queries are fully parameterized, **fundamentally eliminating SQL injection vulnerabilities.**
*   **Secure FileProvider Implementation (Directory Traversal Prevention)**:
    *   CSV exports utilize Android's `FileProvider` to strictly restrict file-sharing access to a temporary cache folder (`exports/`). This prevents directory traversal attacks and locks down app private files.
*   **Private Service Components (No Hijacking)**:
    *   The background telemetry `GPSLoggingService` is strictly set to `exported="false"`, ensuring third-party malicious apps cannot control or intercept logging sessions.
